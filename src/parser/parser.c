...
/*   Updated: 2026/02/18 13:06:18 by mhnatovs         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	init_parser(char *file, t_scene *scene)
{
	...
	if (!ext || ft_strcmp(ext, ".rt") != 0)
		error_exit("Invalid file extension, expected .rt");
	fd = open(file, O_RDONLY);
	if (fd < 0)
		error_exit("Could not open scene file");
	...
	close(fd);
	if (!scene->has_ambient)
		error_exit("Scene is missing Ambient light (A)");
	if (!scene->has_camera)
		error_exit("Scene is missing Camera (C)");
	if (!scene->has_light)
		error_exit("Scene is missing Light (L)");
	return (0);
}
